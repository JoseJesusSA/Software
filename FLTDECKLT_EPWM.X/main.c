 ...
 * Se modifica los registros PMD1, 2, 3, por eso 
 * no respondia ECCP1
 * Version FLTDECKPOT
 *      Se activan  Antirrebotes
 *                  Timer Pot
 *                  Timer 1
 *                  Conversión ADC
 *                  Corrige valor "OFF" & "nOFF"hh
 */
...
//        TMR0L = 0xCA; //10ms@20MHz, 1:16 prescaler
    ...
    if(PIR1bits.TMR1IF)
    {
        ...
        if(IO.POT_FWD.DebounceTimerCounter)
            IO.POT_FWD.DebounceTimerCounter--;
        if(IO.POT_RWD.DebounceTimerCounter)
            IO.POT_RWD.DebounceTimerCounter--;
        if(IO.POT_CSL.DebounceTimerCounter)
            IO.POT_CSL.DebounceTimerCounter--;
        if(IO.POT_INST.DebounceTimerCounter)
            IO.POT_INST.DebounceTimerCounter--;
    ...
    {
        PIR1bits.ADIF=0;
        if(ADCON0bits.CHS==4){              //CSL
            DATAPOT[6]=ADRESH;
            DATAPOT[7]=ADRESL;
        }
        if(ADCON0bits.CHS==2){              //FWD
            ADCON0bits.CHS = 0x04;
            DATAPOT[4]=ADRESH;
            DATAPOT[5]=ADRESL;
            ADCON0bits.GO = 1;
        }
        if(ADCON0bits.CHS==1){              //RWD
            ADCON0bits.CHS = 0x02;
            DATAPOT[2]=ADRESH;
            DATAPOT[3]=ADRESL;
            ADCON0bits.GO = 1;
        }
        if(ADCON0bits.CHS==0){              //INST
            ADCON0bits.CHS = 0x01;
            DATAPOT[0]=ADRESH;
            DATAPOT[1]=ADRESL;
            ADCON0bits.GO = 1;
...
void CAN_Pot(void)
{
    ...
    if(IO.SWI_CSL.Switch.Bits.InputBack==1 && IO.POT_CSL.DebounceTimerCounter == 0){
        IO.POT_CSL.DebounceTimerCounter = IO.POT_CSL.DebounceTimerProg;
        action.UI16 = (UInt16)OVE_FLTDECKLT_POT_CSL_VALUE;
        CAN.TX.SID.UI16 = MyCANIDResp.UI16;
        CAN.TX.RTR = FALSE;
        CAN.TX.DLC = 4;
        CAN.TX.Data[0] = action.UI8[1]; //elemento ID
        CAN.TX.Data[1] = action.UI8[0]; //accion ID
        CAN.TX.Data[2] = DATAPOT[6];
        CAN.TX.Data[3] = DATAPOT[7]; 
        CAN_TXSendStdFrame();
    }
    if(IO.SWI_INST.Switch.Bits.InputBack==1 && IO.POT_INST.DebounceTimerCounter == 0){
        IO.POT_INST.DebounceTimerCounter = IO.POT_INST.DebounceTimerProg;
        action.UI16 = (UInt16)OVE_FLTDECKLT_POT_INST_VALUE;
        CAN.TX.SID.UI16 = MyCANIDResp.UI16;
        CAN.TX.RTR = FALSE;
        CAN.TX.DLC = 4;
        CAN.TX.Data[0] = action.UI8[1]; //elemento ID
        CAN.TX.Data[1] = action.UI8[0]; //accion ID
        CAN.TX.Data[2] = DATAPOT[0]; 
        CAN.TX.Data[3] = DATAPOT[1]; 
        CAN_TXSendStdFrame();
    }
    if(IO.SWI_RWD.Switch.Bits.InputBack==1 && IO.POT_RWD.DebounceTimerCounter == 0){
        IO.POT_RWD.DebounceTimerCounter = IO.POT_RWD.DebounceTimerProg;
        action.UI16 = (UInt16)OVE_FLTDECKLT_POT_RWD_VALUE;
        CAN.TX.SID.UI16 = MyCANIDResp.UI16;
        CAN.TX.RTR = FALSE;
        CAN.TX.DLC = 4;
        CAN.TX.Data[0] = action.UI8[1]; //elemento ID
        CAN.TX.Data[1] = action.UI8[0]; //accion ID
        CAN.TX.Data[2] = DATAPOT[2]; 
        CAN.TX.Data[3] = DATAPOT[3]; 
        CAN_TXSendStdFrame();
    }
    if(IO.SWI_FWD.Switch.Bits.InputBack==1 && IO.POT_FWD.DebounceTimerCounter == 0){
        IO.POT_FWD.DebounceTimerCounter = IO.POT_FWD.DebounceTimerProg;
        action.UI16 = (UInt16)OVE_FLTDECKLT_POT_FWD_VALUE;
        CAN.TX.SID.UI16 = MyCANIDResp.UI16;
        CAN.TX.RTR = FALSE;
        CAN.TX.DLC = 4;
        CAN.TX.Data[0] = action.UI8[1]; //elemento ID
        CAN.TX.Data[1] = action.UI8[0]; //accion ID
        CAN.TX.Data[2] = DATAPOT[4];
        CAN.TX.Data[3] = DATAPOT[5];
        CAN_TXSendStdFrame();
    }
...
//        return;
    ...
    if(CAN.BufferReceived == 1) { // CAN ID de control
        ...
        case (UInt16)OVE_FLTDECKLT_POT_FWD_ANTIREBOTE:               //Antirebote FWD
            ...
            break;
        case (UInt16)OVE_FLTDECKLT_POT_FWD_TIMER:               //timer FWD
            IO.POT_FWD.DebounceTimerProg = CAN.RX0.Data[2];
            break;
        ...
        case (UInt16)OVE_FLTDECKLT_POT_RWD_SWITCHSTAT:                  //Estado RWD
            ...
            CAN_TXSendStdFrame();
            break;
        case (UInt16)OVE_FLTDECKLT_POT_RWD_ANTIREBOTE:            //Antirebote RWD
            IO.SWI_RWD.DebounceTimerProg = CAN.RX0.Data[2];
            break;
        case (UInt16)OVE_FLTDECKLT_POT_RWD_TIMER:               //timer RWD
            IO.POT_RWD.DebounceTimerProg = CAN.RX0.Data[2];
            break;
            
        case (UInt16)OVE_FLTDECKLT_POT_INST_SWITCHSTAT:              //Estado INST
            ...
            CAN_TXSendStdFrame();
            break;
        case (UInt16)OVE_FLTDECKLT_POT_INST_ANTIREBOTE:            //Antirebote INST
            IO.SWI_INST.DebounceTimerProg = CAN.RX0.Data[2];
            break;
        case (UInt16)OVE_FLTDECKLT_POT_INST_TIMER:               //timer INST
            IO.POT_INST.DebounceTimerProg = CAN.RX0.Data[2];
            break;
            
        case (UInt16)OVE_FLTDECKLT_POT_CSL_SWITCHSTAT:              //Estado CSL
            ...
            CAN_TXSendStdFrame();
            break;
        case (UInt16)OVE_FLTDECKLT_POT_CSL_ANTIREBOTE:          //Antirebote CSL
            IO.SWI_CSL.DebounceTimerProg = CAN.RX0.Data[2];
            break;
        case (UInt16)OVE_FLTDECKLT_POT_CSL_TIMER:               //timer CSL
            IO.POT_CSL.DebounceTimerProg = CAN.RX0.Data[2];
            break;
...
void IO_OnInputChanged(void) {
    ...
    if(IO.SWI_FWD.DebounceTimerCounter == 0) {
        ...
        if(IO.SWI_FWD.Switch.Bits.Input != IO.SWI_FWD.Switch.Bits.InputBack) {
            if(IO.SWI_FWD.Switch.Bits.Input == 1)
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_FWD_nOFF;       //on
            else
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_FWD_OFF;
    ...
    if(IO.SWI_RWD.DebounceTimerCounter == 0) {
        ...
        if(IO.SWI_RWD.Switch.Bits.Input != IO.SWI_RWD.Switch.Bits.InputBack) {
            if(IO.SWI_RWD.Switch.Bits.Input == 1)
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_RWD_nOFF;
            else
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_RWD_OFF;
    ...
    if(IO.SWI_CSL.DebounceTimerCounter == 0) {
        ...
        if(IO.SWI_CSL.Switch.Bits.Input != IO.SWI_CSL.Switch.Bits.InputBack) {
            if(IO.SWI_CSL.Switch.Bits.Input == 1)
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_CSL_nOFF ;
            else
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_CSL_OFF;
    ...
    if(IO.SWI_INST.DebounceTimerCounter == 0) {
        ...
        if(IO.SWI_INST.Switch.Bits.Input != IO.SWI_INST.Switch.Bits.InputBack) {
            if(IO.SWI_INST.Switch.Bits.Input == 1)
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_INST_nOFF;
            else
                action.UI16 = (UInt16)OVE_FLTDECKLT_POT_INST_OFF;
