...
void CAN_OnMessage(void) {
    if(CAN.BufferReceived == 1) { // CAN ID de control
        ...
        switch(action.UI16) {
        //------------------------------------------------------------------------------------------
        case (UInt16)FLI_LUZPEDAL1_CFG_CONFIG_PING:
            action.UI16 = (UInt16)FLI_LUZPEDAL1_CFG_CONFIG_PINGRESP;
        ...
        switch(action.UI16) {
        //------------------------------------------------------------------------------------------
        case (UInt16)LT_CM1_PANEL_LAMPS:
            // Actualizar lamparas tubulares. Variar Intensidad.
            //IO_LAMP_INST_POWER_1(0x3FF);
            break;
        //------------------------------------------------------------------------------------------
        case (UInt16)LT_CM1_PANEL_CE:
            // Actualizar lamparas tubulares
            pwm.UI8[1] = CAN.RX1.Data[2];
            pwm.UI8[0] = CAN.RX1.Data[3];
            IO_LAMP_INST_POWER_2(pwm.UI16);
            IO_LAMP_INST_POWER_3(pwm.UI16);
            IO_LAMP_INST_POWER_4(pwm.UI16);
            break;
        //------------------------------------------------------------------------------------------
        case (UInt16)C1I_FLIDECLT_SWI_FLOORPED_FLOPED:
            // Encender lampara de pedales
            IO_LAMP_PEDAL_POWER_1(0x3FF);
            break;
        //-----------------------------------------------------------------------------------------    
        case (UInt16)C1I_FLIDECLT_SWI_FLOORPED_OFF:
            // Apagar lampara de pedales
            IO_LAMP_PEDAL_POWER_1(0);
            break;
        //-----------------------------------------------------------------------------------------    
        default:
            break;
...
void main(void) {
    ...
    canID.UI32 = FLI_LUZPEDAL1_CFG_CONFIG_PINGRESP;
    ...
    canID.UI32 = LT_CM1_PANEL_CE;
    MyCANIDLighting.UI8[1] = canID.UI8[3];
    MyCANIDLighting.UI8[0] = canID.UI8[2];
    
    //la direccion de iluminacion de pedal la tomamos del macro Flight Deck Lt
    canID.UI32 = C1I_FLIDECLT_SWI_FLOORPED_FLOPED;
    MyCANIDPedal.UI8[1] = canID.UI8[3];
    MyCANIDPedal.UI8[0] = canID.UI8[2];
    ...
    CAN.Config.RX1.Filter3.UI16 = MyCANIDPedal.UI16;
